...
if(isset($_POST['motionsubmit'])){
    ...
    $motionsql="UPDATE `motion_uploadform` SET `Title_motion`='$motiontitle',`subtitle_motion`='$motion_subtitle',`Image_motionfile`='$imagemotion' WHERE id=$id";

    $query=mysqli_query($conn,$motionsql);
    if($query){
        echo "<script>alert('Update Sucessfully')</script>";
        echo "<script>location.href='motion-manage.php'</script>";
    }
} 
                    ...
                    <div class="upload-icon">
                        ...
                        <h2>Motion Update</h2>
                            <form action="" method="POST" enctype="multipart/form-data" >
                                <?php
                                    $sql="SELECT * FROM `motion_uploadform` WHERE id=$id LIMIT 1";
                                    $query=mysqli_query($conn,$sql);
                                    $webdisplay=mysqli_fetch_assoc($query);
                                ?>
                                <div class="user-box">
                                    <input type="text" name="motiontitle" value="<?php echo $webdisplay['Title_motion'];?>"required="required">
                                    <label>Title</label>
                                </div>
                                <div class="user-box">
                                    <input type="text" name="motion_subtitle" value="<?php echo $webdisplay['subtitle_motion'];?>" required="required">
                                    <label>Sub Title</label>
                                </div>
                                <div class="user-box">
                                    <input type="file" name="imagemotion" value="<?php echo $webdisplay['Image_motionfile'];?>" required="required">
                                </div>
                                <div class="user-box">
                                ...
                                <input type="submit" name="motionsubmit" value="Update" class="submitbtn">
                            </form>
                    </div>
                </div>
            </div>
