...
if(isset($_POST['websubmit'])){
    ...
    $webtitle= $_POST['web-title'];
    $web_subtitle= $_POST['web-subtitle'];
    

    $websql="UPDATE `web_uploadform`  SET `Title_webimg`='$webtitle',`subtitle_web`='$web_subtitle',`Image_webfile`='$imageweb' WHERE id=$id";

    $query=mysqli_query($conn,$websql);
    if($query){
        echo "<script>alert('Update Sucessfully')</script>";
        echo "<script>location.href='web-manage.php'</script>";
    }
 }   
?>
<!DOCTYPE html>
                ...
                <div class="col-lg-12 col-md-12">
                    ...
                    <div class="upload-icon">
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            <?php
                                $sql="SELECT * FROM `web_uploadform` WHERE id=$id LIMIT 1";
                                $query=mysqli_query($conn,$sql);
                                $webdisplay=mysqli_fetch_assoc($query);
                            ?>
                             <div class="user-box">
                                <input type="text" name="web-title" value="<?php echo $webdisplay['Title_webimg'];?>" required=>
                                <label>Title</label>
                            </div>
                            <div class="user-box">
                                <input type="text" name="web-subtitle" value="<?php echo $webdisplay['subtitle_web'];?>"required>
                                <label>Sub Title</label>
                            </div>
                            <div class="user-box">
                                <input type="file" name="imageweb" value="<?php echo $webdisplay['Image_webfile'];?>"required>
                            </div>
                            <div class="user-box">
                            ...
                            <input type="submit" name="websubmit" value="Update" class="submitbtn">
                        </form>
                    </div>
                </div>
            </div>
