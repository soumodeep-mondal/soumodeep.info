...
if(isset($_POST['graphicssubmit'])){
    ...
    $graphicssql="UPDATE `graphics_uploadform` SET `Title_graphics`='$graphicstitle',`subtitle_graphics`='$graphics_subtitle',`Image_graphicsfile`='$imagegraphics'  WHERE id=$id";

    $query=mysqli_query($conn,$graphicssql);
    if($query){

    echo "<script>alert('Update Sucessfully')</script>";
    echo "<script>location.href='graphics-manage.php'</script>";
    }
}    
?>
<!DOCTYPE html>
                    ...
                    <div class="upload-icon">
                        ...
                        <h2>Graphics update</h2>
                            <form action="" method="POST" enctype="multipart/form-data" >
                                <?php
                                    $sql="SELECT * FROM `graphics_uploadform` WHERE id=$id LIMIT 1";
                                    $query=mysqli_query($conn,$sql);
                                    $graphicsdisplay=mysqli_fetch_assoc($query);
                                ?>
                                <div class="user-box">
                                    <input type="text" name="graphicstitle" value="<?php echo $graphicsdisplay['Title_graphics'];?>" required="required">
                                    <label>Title</label>
                                </div>
                                <div class="user-box">
                                    <input type="text" name="graphics_subtitle" value="<?php echo $graphicsdisplay['subtitle_graphics'];?>" required="required">
                                    <label>Sub Title</label>
                                </div>
                                <div class="user-box">
                                
                                    <input type="file" name="imagegraphics" value="<?php echo $graphicsdisplay['Image_graphicsfile'];?>" >
                                </div>
                                <div class="user-box">
                                ...
                                <input type="submit" name="graphicssubmit" value="Update" class="submitbtn">
                            </form>
                    </div>
                </div>
            </div>
